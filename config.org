...
*** Python
    ...
    Kada se konfigurise Emacs kao Python IDE, potrebno je preduzeti sledece korake:
        - Instalirati Pajton (3.3+ verzije) interpreter i sve potrebne dependency-je( automatski na Linux platformama ), *OBAVEZNO* instalirati *python3-venv* - neophodno da kreiramo virtuelno okruzenje za svaki projekat
        - Instalirati *Pip* da bismo mogli instalirati dodatne pakete
        - Napraviti *virtual environment* pomocu *venv* sa naredbom =python3 -m venv /path/to/new/virtual/environment=, gde je glavni direktorijum virtualnih okruzenja *$HOME(~) u .venvs folderu* , po defaultu( promeniti u konfiguraciji ispod ako je druga putanja u pitanju )
        - Aktivirati virtuelno okruzenje sa =source ~/.venvs/ime-okruzenja/bin/activate=:
          + Kada smo u virtuelnom okruzenju, potrebno je instalirati naredbom =pip3 install ime-paketa=:
            - *Lokalni* Python interpreter za trenutno virtuelno okruzenje
            - *isort* - da soritra importe
            - *Pyflakes* - Proverava greske u fajlu
            - *Pyright* - language server koji koristimo u ovoj konfiguraciji( moze i bilo koji drugi language server )
            - Dodatne potrebne biblioteke za projekat - npr. *Pillow*

    *AKO ZATREBA INSTALIRATI I NODE, NODEJS I NPM!* , u nekim slucajevima trazi da ima ove komponente instalirane
    Takodje, *npm-om* je moguce globalno instalirati *Pyright* server, ali nije preporucljivo, u slucaju da zelimo da promenimo server za neki projekat.

    Doom automatski prepozna sta je projekat( ako ima *.git* fajl npr. ), dodatna konfiguracija najverovatnije nece biti potrebna, sve sto treba je da izaberemo virtuelno okruzenje koje ce koristiti projekat sa =pyvenv-= naredbama. Dodatno konfigurisati sa =SPC p e= da se setupuje *.dir-locals.el* ( iz nekog razloga meni nije radilo ).
...
U *.dir-locals.el* mozemo smestiti konfiguraciju gde nam se nalazi virtuelno okruzenje za dati projekat:

- =(python-mode . ((pyvenv-workon . "ime-environmenta")))=
